...
class RecyclerViewAdapter(val context: Context, val contactList: List<Player>, var onPlayerClickLister: OnPlayerClickLister) : RecyclerView.Adapter<RecyclerViewAdapter.ViewHolder>() {

    var mOnPlayerClickLister = this.onPlayerClickLister
    inner class ViewHolder(view: View, onPlayerClickLister: OnPlayerClickLister) : RecyclerView.ViewHolder(view), View.OnClickListener{
        ...
        fun bindView(contactList: Player, context: Context){
            theContacts.text = contactList.theText
//            contactImage.setImageResource(contactList.image)
            contactImage.setImageResource(contactList.image)
        }

        val onClick = view.setOnClickListener(this)

        override fun onClick(v: View?) {
            vOnPlayerClickLister.onPlayerClick(bindingAdapterPosition)
        }
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.bindView(contactList[position], context)
    }
